		     ...
		     u32 RegisterNum, u16 *PhyDataPtr)
{
	...
	if ((!(XEmacPs_ReadReg(InstancePtr->BaseAddressForMdio,
				XEMACPS_NWSR_OFFSET) &
	...
	XEmacPs_WriteReg(InstancePtr->BaseAddressForMdio,
			   XEMACPS_PHYMNTNC_OFFSET, Mgtcr);

	do {
		Ipisr = XEmacPs_ReadReg(InstancePtr->BaseAddressForMdio,
					  XEMACPS_NWSR_OFFSET);
			IpReadTemp = Ipisr;
		} while ((IpReadTemp & XEMACPS_NWSR_MDIOIDLE_MASK) == 0x00000000U);

	/* Read data */
		*PhyDataPtr = (u16)XEmacPs_ReadReg(InstancePtr->BaseAddressForMdio,
